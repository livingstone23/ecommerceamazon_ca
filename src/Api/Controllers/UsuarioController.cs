...
public class UsuarioController: ControllerBase 
{
    ...
    private IManageImageService _managerImageService;

    public UsuarioController(IMediator mediator, IManageImageService managerImageService)
    {
        _mediator = mediator;
        _managerImageService = managerImageService;
    }
    ...
    public async Task<ActionResult<AuthResponse>> Register([FromForm] RegisterUserCommand request)
    {
        ...
        {
            var resultImage = await _managerImageService.UploadImage(new ImageData
            {
    ...
    public async Task<ActionResult<AuthResponse>> Update([FromForm] UpdateUserCommand request)
    {
        ...
        //Trabajamos la imagen si no es nula
        if(request.Foto != null)
        {
            var resultImage = await _managerImageService.UploadImage(new ImageData
            {
    ...
    [Authorize(Roles = Role.ADMIN)]
    [HttpGet("pagintationAdmin", Name = "PaginationUser")]
    [ProducesResponseType(typeof(PaginationVm<Usuario>), (int)HttpStatusCode.OK)]
    public async Task<ActionResult<PaginationVm<Usuario>>> PaginationUser([FromQuery]PaginationUsersQuery request)
    {
        var paginationUser = await _mediator.Send(request);
